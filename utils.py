...
def render_table_grid(
    ...
    extracted_tables = img.extract_tables(ocr=ocr, borderless_tables=borderless_tables)
    canvas = list(img.images)[0].copy()
    for tb in extracted_tables:
        for row in tb.content.values():
            for cell in row:
                cv2.rectangle(canvas, (cell.bbox.x1, cell.bbox.y1), (cell.bbox.x2, cell.bbox.y2),
                              color, thickness)
    return canvas
